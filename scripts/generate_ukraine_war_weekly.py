...
def _entry_text(entry: Any) -> str:
    parts = [getattr(entry, "title", ""), getattr(entry, "summary", "")]
...
def fallback_summary(items: list[WarItem]) -> dict[str, Any]:
    ...
    executive = [
        f"本期共收集 {len(items)} 条公开报道和分析，来源包括 " + "、".join(f"{k}({v})" for k, v in by_source.most_common()) + "。",
    ...
    for item in items:
        ...
        elif any(k in text for k in ("talk", "deal", "trump", "eu", "policy", "diplomacy")):
    ...
    for title in ["战场态势", "空袭、无人机与导弹", "防务援助与军工", "外交、制裁与政策"]:
        ...
        if not bullets:
            bullets = ["本期公开来源中没有足够材料形成该主题的独立判断。"]
    ...
    key_items = [
        {
            ...
            "source": item.source,
            "url": item.url,
            "why_it_matters": "该条目来自本周公开来源，提供了判断俄乌战争进展的一个观察点。",
        }
        for item in newest
    ]
...
def render_markdown(summary: dict[str, Any], items: list[WarItem], week_id: str, today: str) -> str:
    ...
    lines = [
        ...
        f"# 俄乌战争一周进展: {week_id}",
        "",
        f"> 本期参考 {len(items)} 条公开报道和分析。",
        "",
        "**来源分布**：" + "、".join(f"{source}({count})" for source, count in by_source.most_common()),
        "",
        "## 执行摘要",
        "",
    ]
